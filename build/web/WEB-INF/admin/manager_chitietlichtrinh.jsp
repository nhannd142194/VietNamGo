                        ...
                        <div class="tables">
                            <div class="table-responsive bs-example widget-shadow">
                                ...
                                <table class="table table-bordered" style="margin-top: 20px">
                                    ...
                                    <c:forEach var="l" items="${listChiTiet}">
                                        <c:set var="num" value="${num + 1}" />
                                        <tr> 
                                            <th scope="row"><c:out value="${num}"></c:out></th>
                                            <td>${l.lichTrinh.tenLichTrinh}</td>
                                            <td>${l.user.hoTen}</td>
                                            <td>${l.user.email}</td>
                                            <td>${l.user.dienThoai}</td>
                                            <td>${l.soVe}</td>
                                            <c:if test="${l.tinhTrang == '0'}">
                                                <td style="color: red">Chờ Duyệt</td>
                                            </c:if>
                                                <c:if test="${l.tinhTrang == '1'}">
                                                    <td style="color: green">Đã Duyệt</td>
                                            </c:if>
                                            <td style="width: 100px"><a href="${pageContext.request.contextPath}/admin/quanlydonhang/update?id=${l.idChiTietLichTrinh}&kind=1&idLT=${l.lichTrinh.idLichTrinh}">Duyệt</a> | 
                                           <a href="${pageContext.request.contextPath}/admin/quanlydonhang/update?id=${l.idChiTietLichTrinh}&kind=0&idLT=${l.lichTrinh.idLichTrinh}"> Hủy </a> </td> 
                            ...
                            <div id="page-info" style="float: right">Trang 
                                <c:forEach var="i" begin="1" end="${size}">
                                    <a href="${pageContext.request.contextPath}/admin/quanlydonhang/indexp?p=${i-1}" id="page-info" > <strong>${i}</strong> </a>
                                </c:forEach>
                            </div>
